...
object MinMovieRevenueDataset {
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession
      .builder
      .appName("MinMovieRevenueDataset")
      .master("local[*]")
      .getOrCreate()
    ...
    val minRevenueByYear = ds.groupBy("Year")
      .agg(min("Revenue_millions")
        .alias("minRevenueInMillions"))

    // Filter out all but min revenue by year
    val minRevenue = ds.as("ds")
      .join(minRevenueByYear.as("minData"),$"ds.Year"===$"minData.Year"
      && $"ds.Revenue_millions"===$"minData.minRevenueInMillions")
      .select("ds.Year","Revenue_millions","Rating","Title")
      .orderBy("Year")

    // Collect, format, and print the results
    val results = minRevenue.collect()
