...
object StructuredStreamingAru {
  ...
  def main(args: Array[String]) {
    ...
    val statusCountsDF = logsDF.withColumn("eventTime",current_timestamp())
      .groupBy(window(col ("eventTime"),  "200 second",
       "200 second" ),col("endpoint")).count().orderBy(col("count").desc)

    // Display the stream to the console
    val query = statusCountsDF.writeStream
      .outputMode("complete")
      .format("console").queryName("websiteCounts").start()
